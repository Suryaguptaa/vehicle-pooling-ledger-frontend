    ...
    <>
      <aside className="hidden lg:flex fixed left-0 top-0 h-full w-64 z-40 flex-col" style={{ background: 'rgba(10,10,15,0.95)', borderRight: '1px solid #1e1e2e', backdropFilter: 'blur(20px)' }}>
        ...
        <div className="p-4 m-3 rounded-xl" style={{ background: 'rgba(99,102,241,0.06)', border: '1px solid rgba(99,102,241,0.1)' }}>
          <p className="text-xs text-text-secondary font-body leading-relaxed">Spring Boot API on <span className="text-accent font-mono">:8080</span></p>
        </div>
      </aside>

      <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-40 flex items-center justify-around px-2 py-2" style={{ background: 'rgba(10,10,15,0.98)', borderTop: '1px solid #1e1e2e', backdropFilter: 'blur(20px)' }}>
        {navItems.map(item => (
          <NavLink
            key={item.to}
            to={item.to}
          ...
          >
            {({ isActive }) => (
              <>
                <item.icon size={20} color={isActive ? '#818cf8' : '#3f3f5a'} />
                <span className="text-xs font-body" style={{ color: isActive ? '#818cf8' : '#3f3f5a' }}>{item.label}</span>
              </>
            )}
          </NavLink>
        ))}
